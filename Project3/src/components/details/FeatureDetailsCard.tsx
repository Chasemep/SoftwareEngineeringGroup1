  ...
  return (
    <aside className="surface-card rounded-card border border-slate-200/90 p-4 shadow-panelLg">
      ...
      <div className="mt-3 flex flex-wrap gap-2.5">
        {feature.code ? (
          <span className="rounded-full border border-accent-navy/25 bg-accent-navy-soft/80 px-2.5 py-1 text-xs font-semibold text-accent-navy">
            {feature.code}
          </span>
        ) : null}
      ...
      {feature.locationNote ? (
        <p className="mt-3 rounded-control border border-slate-200 bg-white/88 px-2.5 py-1.5 text-xs leading-relaxed text-text-secondary">
          {feature.locationNote}
        </p>
      ) : null}
    </aside>
